...
- **首次运行与缓存重跑数据 Tab 拆分展示与 Baseline 记录保护**：Dashboard 用例步骤面板在标题右侧增加「首次运行」与「缓存重新运行」Tab 切换按钮，可独立展示不同模式下的子步骤日志、报错信息与步骤截图；当某项运行数据不存在时自动隐藏对应 Tab 按钮。使用缓存重新运行（包含失败场景）彻底隔离对 Baseline 的物理修改，绝不清除或清空首次运行的断点、日志与快照；重跑视图下自动共享回退展示首次运行录制的 API 缓存接口数据。
- **缓存重跑 API 列表 Diff 差异可视化**：在「缓存重新运行」Tab 中自动对比 Baseline 接口列表；若重跑产生了全新的接口请求，URL 呈现 **淡绿色 (#34d399)** 并带有 `+ 新增` 标记；若重跑相比 Baseline 减少（未触发）了某个接口，该接口保留在列表中呈现 **淡红色 (#f87171) + 删格线 (贯穿中划线)** 并带有 `- 未请求` 标记，提升接口变动感知。
